...
export default function App() {
  ...
  useEffect(() => { if (!currentCase || !ACTIVE_STAGES.has(currentCase.stage)) return; const caseId = currentCase.id; const generation = refreshGeneration.current; const source = new EventSource(`/api/cases/${encodeURIComponent(caseId)}/events?after=${currentCase.sequence}`); const schedule = () => { if (refreshTimer.current !== null) return; refreshTimer.current = window.setTimeout(() => { refreshTimer.current = null; void refreshCase(caseId, generation).catch(caught => { if (generation === refreshGeneration.current) setError(String(caught)); }); }, 40); }; source.addEventListener('trace', schedule); const poll = window.setInterval(schedule, 1_000); schedule(); return () => { source.close(); window.clearInterval(poll); if (refreshTimer.current !== null) window.clearTimeout(refreshTimer.current); refreshTimer.current = null; }; }, [currentCase?.id,currentCase?.stage,refreshCase]);
  useLayoutEffect(() => {
